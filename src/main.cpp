...
const string strMessageMagic = "SpreadCoin Signed Message:\n";
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pLastBlock, const CBlockHeader *)
{
    ...
    {
        bnNew += CBigNum().SetCompact(invertCompact(pCurBlock->nBits));
...
bool CBlock::AcceptBlock(CValidationState &state, CDiskBlockPos *dbp)
{
    ...
    if (hash != hashGenesisBlock) {
        ...
        if ((int)nHeight != pindexPrev->nHeight+1)
            return state.DoS(100, error("AcceptBlock() : incorrect height"));
        ...
        if (pcheckpoint && (int)nHeight < pcheckpoint->nHeight)
...
bool ProcessBlock(CValidationState &state, CNode* pfrom, CBlock* pblock, CDiskBlockPos *dbp)
{
    ...
    if (pcheckpoint && pblock->hashPrevBlock != hashBestChain)
    {
        if((pblock->GetBlockTime() - pcheckpoint->nTime) < 0) {
            ...
            return error("ProcessBlock() : block has a time stamp of %" PRI64d " before the last checkpoint of %" PRI64d, pblock->GetBlockTime(), pcheckpoint->nTime);
        }
    }
...
static CBlock getGenesisBlock()
{
    ...
    coinbase.vin[0].scriptSig = CScript() << vector<uint8_t>((const uint8_t*)pszTimestamp, (const uint8_t*)pszTimestamp + strlen(pszTimestamp));
    coinbase.vout.resize(1);
    coinbase.vout[0].nValue = 1 * COIN;
    coinbase.vout[0].scriptPubKey = CScript() << uint160(0) << OP_CHECKSIG;
...
static void IncrementExtraNonce(CBlock* pblock, unsigned int& nExtraNonce)
{
    ...
    ++nExtraNonce;
    pblock->vtx[0].vin[0].scriptSig = (CScript() << CBigNum(nExtraNonce)) + COINBASE_FLAGS;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey* preservekey)
{
    ...
    if (!pblock->CheckProofOfWork())
        return false;

    //// debug print
    printf("SpreadCoinMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SpreadCoinMiner : ProcessBlock, block not accepted");
    }
...
void static SpreadCoinMiner(CWallet *pwallet)
{
    ...
    {
        printf("SpreadCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SpreadCoinMiner, pwallet));
}
